...
public final class ProtocolBuffer {
    ...
    /**
     * Creates a new {@link ProtocolBuffer} with the specified capacity.
     * 
     * @param capacity
     ...
     * @param session
     *            the session we are building this packet header for.
     * @return this protocol buffer.
     */
    public ProtocolBuffer build(int opcode, Session session) {
        writeByte(opcode + session.getEncryptor().getKey());
     ...
     *            the opcode of the packet.
     * @param session
     *            the session we are building this variable packet header for.
     * @return this protocol buffer.
     */
    public ProtocolBuffer buildVar(int opcode, Session session) {
        build(opcode, session);
     ...
     *            the opcode of the packet.
     * @param session
     *            the session we are building this variable short packet header
     *            for.
     * @return this protocol buffer.
     */
    public ProtocolBuffer buildVarShort(int opcode, Session session) {
        build(opcode, session);
        variableLengthPos = buf.position();
        writeShort(0);
        return this;
    ...
    /**
     * Determines if this buffer is a packet or not.
     * 
     * @return true if this buffer is a packet.
     */
    ...
    /**
     * Sends the packet to the session that was used to encrypt the header.
     * 
     * @return this protocol buffer.
     */
